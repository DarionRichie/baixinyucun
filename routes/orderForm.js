...
router.get('/', function(req, res, next) {
    ...
    try {//使用openid进行某一个人的直接访问;
        ...
        testdb.api_db_query1(JSON.stringify(options), "orderForm").then((ress) => {
            res.json(ress);
        });
    }catch(err){
        console.log(err);
...
router.post('/new',function(req,res){//新建一个对应的房间
    try{
        ...
        //构建插入对象
        let objparse = obj;
        testdb.api_db(JSON.stringify(objparse),"orderForm").then((response)=>{
...
router.post('/delete',function(req,res,next){
    try{
        ...
        testdb.api_db_delete(JSON.stringify(option),"orderForm").then((response)=>{
...
router.post('/fix',function(req,res,next){
    try{
        ...
        let option = {"_id":objparse["_id"]};
        console.log(objparse);
        testdb.api_db_update(JSON.stringify(option),JSON.stringify(objparse),"orderForm").then((response)=>{
...
router.post('/deleteAll',function(req,res,next){
    try{
        ...
        testdb.api_db_delete(JSON.stringify({}),"orderForm",'all').then((response)=>{
