...
//数据库的查询删除等都在一个js文件里面，只需要传入对应的集合就可以进行删除等;
var request = {
	api_db: function (option,setname){
		return new Promise(function(resolve,reject,typeofDatabase){
	...
	api_db_delete: function (option,setname,status){
		return new Promise(function(resolve,reject){
			console.log("使用了api请求");
			var ACCESS_TOKEN = "";
			const appid = "wx104ea7e4f90396f7";
			...
			axios.get(access_tokenurl).then((response)=>{
				...
				data = option;
				if(status == 'all')
					queryurl = "db.collection(\""+setname.toString()+"\").where({id:_.exists(true)}).remove()";
				else
